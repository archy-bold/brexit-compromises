...
do
    ...
    curl http://lda.data.parliament.uk/commonsdivisions/id/$i.json | \
        jq -c . | docker exec -i bc-broker kafka-console-producer --broker-list broker:9092 --topic raw-votes;
    # Publish the individual votes
    curl http://lda.data.parliament.uk/commonsdivisions/id/$i.json | \
        jq -c '.result.primaryTopic.vote[]' | \
        sed "s/^/$i|/" | \
        docker exec -i bc-broker kafka-console-producer --broker-list broker:9092 --topic individual-votes --property "parse.key=true" --property "key.separator=|"
done
